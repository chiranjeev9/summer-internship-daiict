<?php
		...
		else if($_POST['Type'] == "Occation")
		{
			...
			$res=mysqli_query($Create_Connection, "SELECT * FROM `occationm` where occationname='".$occationname."'");
			$rowcount=mysqli_num_rows($res);
			...
			{
				$Add_Data = "INSERT INTO `occationm` (`occationname`) VALUES ('".mysqli_real_escape_string($Create_Connection, $occationname)."')";
			
				$Fire_Insert_Query = mysqli_query($Create_Connection, $Add_Data);

				if(mysqli_affected_rows($Create_Connection) == 1)
				{
					$Send_Response['Status_Type'] = "OCCATION_ADDING_SUCCESSFUL";
					$Send_Response['Status_Message'] = "'".$occationname."' occation added successfully.";
				}
				else
				{
					$Send_Response['Status_Type'] = "OCCATION_ADDING_FAILED";
					$Send_Response['Status_Message'] = "Failed to add '".$occationname."' occation.\n\nPlease try again later.";
				}
			}
