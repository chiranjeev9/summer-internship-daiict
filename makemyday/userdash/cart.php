<?php
    ...
    //echo "<script>alert('".$_SESSION['CUser_ID']."');</script>";
    $cartd = mysqli_query($Create_Connection,"Select * from cartm, cartt, productm where productm.productid = cartt.productid and cartt.cartid = cartm.cartid and cartm.userid = '".$_SESSION['CUser_ID']."'");
    $res = mysqli_query($Create_Connection,"Select s.stateid,s.statename,c.countryname from statem s inner join countrym c on c.countryid=s.countryid where s.isactive = 1 and c.isactive = 1 ORDER BY s.stateid");
              ...
              <form role="form" method="POST" action="">
                <div class="card-body">
                  ...
                  <div class="form-group" style="display:">
                    ...
                    <input type="number" class="form-control" id="quantity" name="Quantity" max = 10 min = 1 placeholder="Enter Quantity">
                  </div>
                ...
                <div class="card-footer">
                  <button type="submit" id="Update" name="Update" class="btn btn-primary btn-warning fas fa-edit"> Update</button>
                  <button type="submit" id="Delete" name="Delete" class="btn btn-primary btn-danger fas fa-trash-alt"> Delete</button>
                  <button type="Reset" id="Cancel" class="btn btn-primary btn-info fas fa-stop-circle"> Cancel</button>
                </div>
              </form>
            </div>
            <!-- /.card -->
            ...
            <div class="card">
              ...
              <div class="card-body table-responsive p-0" id="Data">
                <table class="table table-hover table-bordered" id="Cart-table">
                  ...
                  <tbody>
                    <?php
                      while($row1=mysqli_fetch_array($cartd))
                      {
                        echo '
                              <tr>
...
<script>
        function printDiv() { 
            ...
            a.document.write('<body > <h1>Cart Report<br><hr>'); 
        ...
        function exportTableToExcel(tableID, filename = 'Cartreport'){
            //$('table tr td:eq(2)').hide();
            //$('table tr th:eq(2)').hide();
            var downloadLink;
...
<!--<script>
    $(function()
    {
      ...
      $('#Save').click(function()
      {
        ...
        else if(State_Name.length < 2)
        {
          alert('Entered State name is too short to use.\n\nPlease enter proper name.');
        }
        else if(isNaN(State_Name) == false)
        {
          alert('Your State name contains numbers which is not allowed.\n\nPlease enter only alphabets.');
        }
        else if(Country_Name == "Select Country")
        {
          alert('Please select your Country name.'); 
        }
        else
        {
          var State_Data_String = 'Statename='+State_Name+'&Countryname='+Country_Name+'&Type=State';
          
          $.ajax
          ({
            type: "POST",
            url: "business/add.php",
            data: State_Data_String,
            cache: false,
            async: true,
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'STATE_ADDING_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'STATE_ALREADY_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'STATE_ADDING_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
      ...
      $('#Update').click(function()
      {
        ...
        {
          var State_Data_String = 'statename='+State_Name+'&stateid='+State_ID+'&countryname='+Country_Name+'&Type=State';
          $.ajax
          ({
            type: "POST",
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'STATE_UPDATED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              ...
              else if(Received_Data.Status_Type == 'STATE_UPDATION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
        ...
        else
        {
          ...
          ({
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'STATE_DELETED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'STATE_DOES_NOT_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'STATE_DELETION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
...
<?php
    ...
    if(isset($_POST['Update']))
    {
      if($pname == "" || $pname == null)
      {
          echo "<script>alert('Please Select Product to Update');</script>";   
      }
      else if($qty=="" || $qty==null)
      {
          echo "<script>alert('Please Select valid Quantity');</script>";
      }
      else
      {
        mysqli_query($Create_Connection,"update cartt set productid='".$pid."', qty='".$qty."'");
      }
    }
