<?php
    ...
    $query = "Select * from occationm where isactive = 1 ORDER BY occationid";
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Occation</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Manage Occation</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form">
                <div class="card-body">
                  <div class="form-group" style="display:">
                    ...
                    <input type="hidden" class="form-control" id="Occationid">
                    <input type="text" class="form-control" id="Occationname" placeholder="Enter Occation Name">
                  </div>
            ...
            <div class="card card-success">
              <div class="card-header">
                <h3 class="card-title">Occation Details</h3>
              </div>
              <div class="card-body">
          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body table-responsive p-0" id="Data">
                <table class="table table-hover table-bordered" id="Occation-table">
                  <thead>
                    <tr>
                      <th>Occation ID</th>
                      <th>Occation Name</th>
                  ...
                  <tbody>
                    <?php
                      while($row=mysqli_fetch_array($res))
                      {
                        echo '
                              <tr>
                                <td>'.$row["occationid"].'</td>
                                <td>'.$row["occationname"].'</td>
...
<script>
        function printDiv() { 
            ...
            a.document.write('<body > <h1>Occation Report<br><hr>'); 
        ...
        function exportTableToExcel(tableID, filename = 'Occationreport'){
            $('table tr td:eq(2)').hide();
            $('table tr th:eq(2)').hide();
            var downloadLink;
...
<script>
                ...
                var table = document.getElementById('Occation-table');
                for(var i = 1; i < table.rows.length; i++)
                {
                    table.rows[i].onclick = function()
                    {
                         //rIndex = this.rowIndex;
                         document.getElementById("Occationid").value = this.cells[0].innerHTML;
                         document.getElementById("Occationname").value = this.cells[1].innerHTML;
                         window.scrollTo(0, 0);
                    };
                }
</script>
...
<script>
    $(function()
    {
      ...
      $('#Save').click(function()
      {
        var Occation_Name = $('#Occationname').val();
        if(Occation_Name == "" || Occation_Name == null)
        {
          alert('Please enter your Occation name.');
        }
        else if(Occation_Name.length < 2)
        {
          alert('Entered Occation name is too short to use.\n\nPlease enter proper name.');
        }
        else if(isNaN(Occation_Name) == false)
        {
          alert('Your Occation name contains numbers which is not allowed.\n\nPlease enter only alphabets.');
        }
        else
        {
          var Occation_Data_String = 'Occationname='+Occation_Name+'&Type=Occation';
          
          $.ajax
          ({
            type: "POST",
            url: "business/add.php",
            data: Occation_Data_String,
            cache: false,
            async: true,
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'OCCATION_ADDING_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'OCCATION_ALREADY_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'OCCATION_ADDING_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
      ...
      {
        var Occation_Name = $('#Occationname').val();
        var Occation_ID = $('#Occationid').val();
        if(Occation_Name == "" || Occation_Name == null)
        {
          alert('Please Select Occation to update.');
        }
        else if(Occation_Name.length < 2)
        {
          alert('Entered Occation name is too short to use.\n\nPlease enter proper name.');
        ...
        {
          var Occation_Data_String = 'Occationname='+Occation_Name+'&Occationid='+Occation_ID+'&Type=Occation';
          $.ajax
          ({
            type: "POST",
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'OCCATION_UPDATED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              ...
              else if(Received_Data.Status_Type == 'OCCATION_UPDATION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
      ...
      $('#Delete').click(function()
      {
        ...
        if(Occation_Name == "" || Occation_Name == null)
        {
          alert('Please Select Occation to delete.');
        }
        else
        {
          var Occation_Data_String = 'Occationname='+Occation_Name+'&Occationid='+Occation_ID+'&Type=Occation';
          ...
          ({
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'OCCATION_DELETED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'OCCATION_DOES_NOT_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'OCCATION_DELETION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
...
<!--  <script>  
      $(document).ready(function(){  
        $('#Occation-table').Tabledit({
          url:'action.php',
          columns:{
          identifier:[0, "Occationid"],
          editable:[[1, 'Occationname']]
        },
        restoreButton:false,
