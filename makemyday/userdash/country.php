<?php
    ...
    $query = "Select * from countrym where isactive = 1 ORDER BY countryid";
...
<script>
        function printDiv() { 
            ...
            a.document.write('<body > <h1>Country Report<br><hr>'); 
        ...
        function exportTableToExcel(tableID, filename = 'countryreport'){
            $('table tr td:eq(2)').hide();
            $('table tr th:eq(2)').hide();
            var downloadLink;
...
<script>
                ...
                for(var i = 1; i < table.rows.length; i++)
                {
                    table.rows[i].onclick = function()
                    {
                         ...
                         document.getElementById("countryid").value = this.cells[0].innerHTML;
                         document.getElementById("countryname").value = this.cells[1].innerHTML;
                         window.scrollTo(0, 0);
                    };
                }
</script>
...
<script>
    $(function()
    {
      ...
      $('#Save').click(function()
      {
        ...
        {
          alert('Entered country name is too short to use.\n\nPlease enter proper name.');
        }
        else if(isNaN(Country_Name) == false)
        {
          alert('Your country name contains numbers which is not allowed.\n\nPlease enter only alphabets.');
        }
        else
        {
          var Country_Data_String = 'countryname='+Country_Name+'&Type=Country';
          
          $.ajax
          ({
            type: "POST",
            url: "business/add.php",
            data: Country_Data_String,
            cache: false,
            async: true,
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'COUNTRY_ADDING_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'COUNTRY_ALREADY_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'COUNTRY_ADDING_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
      ...
      $('#Update').click(function()
      {
        ...
        {
          var Country_Data_String = 'countryname='+Country_Name+'&countryid='+Country_ID+'&Type=Country';
          $.ajax
          ({
            type: "POST",
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'COUNTRY_UPDATED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              ...
              else if(Received_Data.Status_Type == 'COUNTRY_UPDATION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
        ...
        else
        {
          ...
          ({
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'COUNTRY_DELETED_SUCCESSFUL')
              {
                alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);
              }
              else if(Received_Data.Status_Type == 'COUNTRY_DOES_NOT_EXISTS')
              {
                alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'COUNTRY_DELETION_FAILED')
              {
                alert(Received_Data.Status_Message);
              }
            }
