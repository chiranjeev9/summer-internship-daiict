<?php
    ...
    $res = mysqli_query($Create_Connection,"Select * from userm where isactive = 1 ORDER BY userid");
?>
<!DOCTYPE html>
<html>
<head>
              ...
              <form role="form">
                <div class="card-body">
                  <div class="form-group" style="display:">
                    ...
                    <input type="text" class="form-control" id="Username" placeholder="User's Full Name" disabled=true>
                  </div>
                ...
                <div class="card-footer">
                  <button type="button" id="Save" class="btn btn-primary btn-success fas fa-save"> Add New</button>
                  <button type="button" id="Update" class="btn btn-primary btn-warning fas fa-edit"> Update/Delete</button>
                  <button type="Submit" id="Cancel" class="btn btn-primary btn-info fas fa-stop-circle"> Cancel/Refresh</button>
                </div>
              </form>
            </div>
            <!-- /.card -->
                      ...
                      while($row=mysqli_fetch_array($res))
                      {
                        echo '
                              <tr>
                                ...
                                <td>'.$row["emailid"].'</td>
                                <td>'.$row["addressline1"].' '.$row['addressline2'].' '.$row["landmark"].' - '.$row["pincode"].'</td>
                                <td>'.$row["usertype"].'</td>
                                <td><img src=img/'.$row["userphoto"].'></td>
...
<script>
    $(function()
    {
      ...
      $('#Update').click(function()
      {
        ...
        {
          var User_Data_String = 'username='+User_Name+'&userid='+User_ID+'&Type=User';
          $.ajax
          ({
            type: "POST",
            url: "users.php",
            ...
            { 
              ...
              if(Received_Data.Status_Type == 'USER_UPDATED_SUCCESSFUL')
              {
               /* alert(Received_Data.Status_Message);
                location.reload(true);
                window.scrollTo(0, 700);*/
              }
              else if(Received_Data.Status_Type == 'USER_ALREADY_EXISTS')
              {
                //alert(Received_Data.Status_Message);
              }
              else if(Received_Data.Status_Type == 'USER_UPDATION_FAILED')
              {
                //alert(Received_Data.Status_Message);
              }
            }
